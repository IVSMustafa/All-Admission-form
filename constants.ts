...
// One-to-One Schooling only allows KG1 to Grade 7
...
export const getGradeValue = (grade: string): number => {
  const map: Record<string, number> = {
    'KG1': 1, 'KG2': 2,
    'Grade 1': 3, 'Grade 2': 4, 'Grade 3': 5, 'Grade 4': 6, 'Grade 5': 7, 'Grade 6': 8, 'Grade 7': 9,
...
export const COUNTRIES = [
  'Saudi Arabia', 'United Arab Emirates', 'Pakistan', 'Qatar', 'Kuwait', 'Oman', 'Bahrain', 'United Kingdom', 'USA', 'Canada', 'Australia', 'Other'
];

// Country phone data for WhatsApp validation
export const COUNTRY_PHONE_DATA: Record<string, { code: string; flag: string; digits: number; example: string }> = {
  // --- Gulf & Middle East ---
  ...
  'Ukraine': { code: '+380', flag: '🇺🇦', digits: 9, example: 'XX XXXXXXX' },
  ...
  // --- Fallback ---
  'Other': { code: '+', flag: '🌍', digits: 10, example: 'XXXXXXXXXX' }
};

// Helper to format phone for WhatsApp
export const formatPhoneForWhatsApp = (country: string, phone: string): string => {
  const countryData = COUNTRY_PHONE_DATA[country] || COUNTRY_PHONE_DATA['Other'];
  const cleanPhone = phone.replace(/\D/g, '');
  return `${countryData.code}${cleanPhone}`;
};

// Helper to validate phone length
export const validatePhoneLength = (country: string, phone: string): boolean => {
  const countryData = COUNTRY_PHONE_DATA[country] || COUNTRY_PHONE_DATA['Other'];
  const cleanPhone = phone.replace(/\D/g, '');
  return cleanPhone.length === countryData.digits;
};

export const IGCSE_SUBJECTS = [
  'English', 'Mathematics', 'Biology', 'Physics', 'Chemistry', 'Pakistan Studies', 'Computer Science', 'ICT', 'Islamic Studies', 'Accounting', 'Economics', 'Business Studies'
];
...
// Quran class time slots (30-minute classes, available 24/7)
