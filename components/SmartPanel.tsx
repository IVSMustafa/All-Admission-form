...
const CircularProgress: React.FC<{ percent: number }> = ({ percent }) => {
  const animatedPercent = useAnimatedNumber(percent, 1300);
  ...
  return (
    <div className="sp-circleWrap" aria-label={`${animatedPercent}% complete`}>
      <div className="sp-circleHalo" />
      ...
      <svg
        ...
        className="sp-circleSvg"
      >
        <defs>
          <linearGradient id="sp-progressGradient" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <circle
          ...
          r={radius}
          fill="none"
          stroke="url(#sp-progressGradient)"
          strokeWidth={stroke}
          strokeDasharray={circumference}
          strokeDashoffset={offset}
          strokeLinecap="round"
          className="sp-circleProgress"
        />
      </svg>

      <div className="sp-circleLabel">
        <span key={percent} className="sp-circlePercent sp-percentPop">
          {animatedPercent}%
        </span>
      </div>
    </div>
...
const SmartPanel: React.FC<SmartPanelProps> = ({ data, step }) => {
  const [mobileDrawerOpen, setMobileDrawerOpen] = useState(false);
  const [showDesktopPanel, setShowDesktopPanel] = useState(false);
  const [compactDesktopPanel, setCompactDesktopPanel] = useState(false);
...
useEffect(() => {
  const updatePanelMode = () => {
    ...
    const canShowDesktop = vw >= 1024;
    const shouldUseCompact = vw < 1440 || vh < 860;

    setShowDesktopPanel(canShowDesktop);
    setCompactDesktopPanel(shouldUseCompact);
  };
      ...
      {step >= 1 && data.leadType === LeadType.FULL_TIME && grade8PlusStudents.length > 0 && (
        <InsightCard icon={<GraduationCap className="w-5 h-5" />} title="Curriculum Options" variant="purple" delay={300}>
          <div className="sp-miniStack">
            <div className="sp-miniText">For Grade 8+ students, we offer:</div>
    ...
    <>
      ...
      <div className="sp-progressBar">
        ...
        <div className="sp-progressTrack">
          ...
          <div className="sp-progressShine" style={{ width: `${progress}%` }} />
        </div>
      </div>
  ...
  return (
    <>
      <style>{`
        ...
        .sp-percentPop {
          animation: spPercentPop 0.5s cubic-bezier(0.22, 1, 0.36, 1);
        }

        @keyframes spPercentPop {
        ...
        .sp-timelineLine {
          ...
          width: 2.5px;
          bottom: 0;
          background: linear-gradient(to bottom, rgba(29,111,206,0.26), rgba(29,111,206,0.06));
          border-radius: 2px;
          overflow: hidden;
        }

        .sp-timelineLine::after {
          content: "";
          position: absolute;
          inset: 0;
          ...
          animation: spLineGlow 2.6s ease-in-out infinite;
        }

        @keyframes spLineGlow {
        ...
        .sp-progressFill {
          ...
          height: 100%;
          border-radius: 999px;
          background: linear-gradient(90deg, #1d6fce 0%, #38bdf8 55%, #7dd3fc 100%);
          transition: width 1.1s cubic-bezier(0.22, 1, 0.36, 1);
          box-shadow:
            0 0 18px rgba(29,111,206,0.28),
            0 0 8px rgba(56,189,248,0.20);
        }

        .sp-progressFill::after {
          content: "";
          position: absolute;
          inset: 0;
        ...
        .sp-drawer {
          ...
          left: 0;
          right: 0;
          background: linear-gradient(180deg, rgba(255,255,255,0.74), rgba(244,248,255,0.58));
...
.sp-panel-compact {
  ...
  border-radius: 22px;
        ...
        .sp-panel-compact .sp-header {
          padding: 18px 18px;
        }

        .sp-panel-compact .sp-brand {
          gap: 10px;
        }

        .sp-panel-compact .sp-brandIcon {
          padding: 8px;
        }

        .sp-panel-compact .sp-brandTitle {
          font-size: 12px;
          letter-spacing: 1px;
        }

        .sp-panel-compact .sp-brandSubtitle {
          font-size: 11px;
        }

        .sp-panel-compact .sp-circleWrap {
          width: 74px;
          height: 74px;
        }

        .sp-panel-compact .sp-circlePercent {
          font-size: 15px;
        }

        .sp-panel-compact .sp-timeline {
          padding: 16px 18px;
        }

        .sp-panel-compact .sp-timelineStep {
          padding-bottom: 18px;
        }

        .sp-panel-compact .sp-timelineIcon {
          width: 36px;
          height: 36px;
          font-size: 13px;
        }

        .sp-panel-compact .sp-timelineLine {
          left: 17px;
          top: 36px;
        }

        .sp-panel-compact .sp-timelineContent {
          margin-left: 12px;
          padding-top: 5px;
        }

        .sp-panel-compact .sp-timelineTitle {
          font-size: 12px;
        }

        .sp-panel-compact .sp-timelineSub {
          font-size: 10px;
        }

        .sp-panel-compact .sp-progressBar {
          padding: 0 18px 16px;
        }

        .sp-panel-compact .sp-advisorContent {
          padding: 0 18px 18px;
          gap: 12px;
        }

        .sp-panel-compact .sp-tipCard,
        .sp-panel-compact .sp-insight {
          padding: 13px 14px;
          border-radius: 16px;
        }

        .sp-panel-compact .sp-tipIcon {
          padding: 7px;
        }

        .sp-panel-compact .sp-tipHeader {
          font-size: 11px;
        }

        .sp-panel-compact .sp-tipDescription,
        .sp-panel-compact .sp-insightBody {
          font-size: 11px;
          line-height: 1.55;
        }

        .sp-panel-compact .sp-insightHead {
          gap: 10px;
          margin-bottom: 8px;
        }

        .sp-panel-compact .sp-insightIco {
          width: 32px;
          height: 32px;
          border-radius: 10px;
        }

        .sp-panel-compact .sp-insightTitle {
          font-size: 12px;
        }

        .sp-panel-compact .sp-chip {
          font-size: 10px;
          padding: 5px 10px;
        }

        .sp-panel-compact.sp-hangWrap .sp-hangDecor {
          top: -82px;
          height: 82px;
        }

        .sp-panel-compact.sp-hangWrap .sp-rope {
          height: 78px;
        }

        .sp-panel-compact.sp-hangWrap .sp-ring {
          top: 72px;
          width: 15px;
          height: 15px;
        }

        .sp-panel-compact.sp-hangWrap .sp-hook {
          top: 76px;
          width: 12px;
          height: 12px;
        }
      ...
      {showDesktopPanel && (
        <div className="block">
          <div className={`sp-hangWrap ${compactDesktopPanel ? "sp-panel-compact" : ""}`}>
            ...
            <div className={`sp-panel sp-hangSway ${compactDesktopPanel ? "sp-panel-compact" : ""}`}>
              <PanelInner />
            </div>
          </div>
        </div>
